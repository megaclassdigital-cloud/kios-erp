    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/40 p-4">
          <div className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-xl bg-card p-5 shadow-xl">
            ...
            {data && (
              <div className="space-y-4 text-sm">
                  ...
                  <tbody>
                    {data.sale.items.map((item) => (
                      <tr key={item.id} className="border-t border-border">
                        ...
                        <td className="py-1.5 text-muted-foreground tabular-nums">{Number(item.quantity)}</td>
                        <td className="py-1.5 text-muted-foreground tabular-nums">{formatRupiah(item.unitPriceAtSale)}</td>
                        <td className="py-1.5 text-muted-foreground tabular-nums">{formatRupiah(item.costPriceAtSale)}</td>
                        <td className="py-1.5 font-medium text-foreground tabular-nums">{formatRupiah(item.subtotal)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div className="flex justify-between border-t border-border pt-2 font-semibold text-foreground">
                  ...
                  <span className="tabular-nums">{formatRupiah(data.sale.grandTotal)}</span>
                </div>

                <div>
                  <h3 className="mb-1 text-xs font-semibold text-muted-foreground">MUTASI STOK TERKAIT</h3>
                  {data.stockMovements.length === 0 ? (
                    <p className="text-xs text-muted-foreground">Tidak ada mutasi stok (semua item layanan).</p>
                  ) : (
